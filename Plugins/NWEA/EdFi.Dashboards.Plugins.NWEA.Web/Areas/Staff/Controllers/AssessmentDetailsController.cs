...
namespace EdFi.Dashboards.Plugins.NWEA.Web.Areas.Staff.Controllers
{
                                ...
                                int localEducationAgencyId, string subjectArea, string assessmentSubType)
        {
            ...
            foreach (var s in results.Students)
            {
                ...
                foreach (var o in results.ObjectiveTitles)
                {
                    if (isBenchmark)
                    {
                        var cell = new GoalStrandCellItem<int> {DV = String.Empty, V = -1};
            ...
            {
                model.LegendViewNames = new List<string> { "GoalStrands" };
            }
